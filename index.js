...
function customObjectFromEntries(entries) {
  if (Array.isArray(entries) && entries.length > 0) {
    let value2string = entries.map(([key, value]) => !isNaN(key) ? [key, key.toString()] : [key, value])
    return Object.fromEntries(value2string)
  } else {
...
function checkProperty(obj, prop) {
  if (typeof obj === 'object' && !Array.isArray(obj) && obj !== null && typeof prop === "string") {
...
function filterObjectsByKey(arr, key) {
  if (Array.isArray(arr)) {
    return arr.filter(elem => elem.hasOwnProperty(key))
  } else {
    return []
  }
...
function getObjectValuesSum(obj) {
  if (typeof obj === 'object' && !Array.isArray(obj) && obj !== null) {
    return Object.values(obj).reduce((sum, elem) => sum += elem, 0)
  } else {
    return false
  }
